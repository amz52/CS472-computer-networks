...
class Server:
    def __init__(self, port, logger):
        ...
        self.serverconf = self.cwd + '\\server.conf'
        self.ssl_bool = None

        if not os.path.isfile(self.serverconf):
            print('Error: Server config file not in current directory.')
            sys.exit(1)
        elif not os.path.isfile(self.usersfile):
            print('Error: Username file not in current directory.')
            sys.exit(1)

        with open(self.serverconf, 'r') as conf:
            settings = conf.readlines()

        self.record.log('Reading config file.')
        for line in settings:
            setting = line.split(':')
            if setting[0] is 'port':
                self.portmode = setting[1]
            if setting[0] is 'pasv':
                self.pasvmode = setting[1]
            if setting[0] is 'ssl':
                self.ssl_bool = setting[1]
        self.record.log('Settings have been read.')
        print('Settings have been set.')

        if self.pasvmode == 'NO' and self.portmode == 'NO':
            print('Error: Both PASV and PORT modes are disabled. Cannot proceed.')
            sys.exit(1)

        self.serversocket = FtpSocket(self.port, self.record, self.ssl_bool)
        self.clientele = []
    ...
    def ignition(self):
        ...
        while True:
            ...
            # implemting client specific logger
...
class Thread:
    ...
    def __init__(self, connection, address, logger, server):
        ...
        self.record = logger
        self.server = server
        self.client_record = Logger(self.address[0])
    ...
    def start(self):
        ...
        while True:
            try:
                ...
                self.client_record.log('Connection lost or aborted with Error: ' + str(e))
                break
            if not response:
                self.record.log('No response...')
                self.client_record.log('No response...')
                ...
                self.client_record.log('Processing ' + response + '...')
        ...
        self.client_record.log('Thread closed')
    ...
    def CDUP(self):
        ...
        self.client_record.log('CDUP Response: ' + response)
    ...
    def CWD(self, options='.'):
        ...
        self.client_record.log('CWD Response: ' + response)
        return response, valid_cwd

    def EPRT(self, arguments):
        delimiter = arguments[0]
        eprt_arguments = arguments[1:-1]
        list(filter(delimiter.__ne__, eprt_arguments))

        ip_type = eprt_arguments[0]
        if ip_type != 1 and ip_type != 2:
            response = 'Network Protocol not supported, use (1, 2)\n'

        ip = eprt_arguments[0]
        port = eprt_arguments[1]

        self.data_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.data_socket.bind((ip, port))
        self.data_socket.listen(1)

        if self.server.ssl_bool:
            self.data_socket = ssl.wrap_socket(self.data_socket, ssl_version=ssl.PROTOCOL_TLS,
                                               keyfile=key, cert_reqs=ssl.CERT_OPTIONAL,
        ...
        self.client_record.log('EPRT Response: ' + response)
        return response, eprt_success

    def EPSV(self):
        pass
    # couldnt understand how to implement
    ...
    def HELP(self):
        ...
        self.client_record.log('HELP Response: OK\n')
    ...
    def LIST(self, options='.'):
        ...
        self.client_record.log('LIST Response: ' + response)
            ...
            if self.user_variable in user:
                ...
                if option == password.rstrip():
                    ...
                    if self.attempts == 3:
                        ...
                        self.client_record.log('ERROR: ' + response)
                        sys.exit(1)
                    else:
                        response = 'Password incorrect. To try again enter PASS\n'
                        self.attempts += 1
                        self.record.log('BAD PASSWORD')
                        self.client_record.log('BAD PASSWORD')

        self.record.log('PASS Response: ' + response)
        self.client_record.log('PASS Response: ' + response)
    ...
    def PASV(self):
        ...
        if self.server.ssl_bool:
            self.server_socket = ssl.wrap_socket(self.server_socket, ssl_version=ssl.PROTOCOL_TLS,
                                                 keyfile=key, cert_reqs=ssl.CERT_OPTIONAL,
                                                certfile=certificate)
        ...
        self.client_record.log('Opened socket %s:%s', ip, port)
    ...
    def PORT(self, args):
        ...
        self.client_record.log('Custom addr and port ' + self.data_addr, + ':' + self.data_port)
    ...
    def RETR(self, file):
        ...
        self.data_socket = FtpSocket(self.data_port, self.record, self.server.ssl_bool)
        ...
        self.client_record.log('RETR Response: ' + response)
    ...
    def USER(self, option):
        ...
        self.client_record.log('USER Response: ' + response)
...
class FtpSocket:
    ...
    def __init__(self, port, logger, ssl_bool):
        self.host = '127.0.0.1'
        self.port = port
        self.record = logger
        self.ssl = ssl_bool
        ...
        if ssl_bool is True:
            self.serversocket = ssl.wrap_socket(self.serversocket, ssl_version=ssl.PROTOCOL_TLS,
                                                keyfile=key, cert_reqs=ssl.CERT_OPTIONAL,
                                                certfile=certificate)
